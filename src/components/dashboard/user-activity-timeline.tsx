...
async function fetchUserActivityLogs(): Promise<ActivityLogEntry[]> {
  const response = await fetch("/api/activity-logs?userId=current")
  if (!response.ok) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Your recent activity and task updates
          </CardDescription>
        </CardHeader>
        <CardContent>
